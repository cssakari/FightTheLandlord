        ...
        if (history[i].upup.empty())
        {
            ...
            for (int ii = 0; ii <= 2 && ii != player; ++ii)
            {
                ...
                if (ii < 2)
                {
                    ...
                    if (temp.comboType != CardComboType::PASS)
                    {
                        maynotHave[player][(int)temp.comboType] = true;
                        break;
                    }
                }
                else
...
int *play(CARD_ITERATOR begin, CARD_ITERATOR end)
{
    ...
    {
        int temp = int(*begin);
        myResponsee.append(temp);
    }
    response.append(myResponse);
...
void Split_Combo()
{
    ...
    for (Level i = 0; i < 12 && avai.size(); i++)
    {
        ...
        case 1:
            ...
            if (s < 0 && avai.size())
            {
                ...
                {
                    BestCombo.push_back(new CardCombo(avai[0].begin(), avai[0].end()));
                    avai.erase(avai.begin()); // 填满则装入
                }
            }
            break; // 带走成为Combo
        case 2:
                    ...
                    case 2:
                        for (int k = i + 1; k < 8; k++)
                        {
                            ...
                            { // 同为对子且差距较小
                                ...
                                BestCombo.push_back(new CardCombo(avai[j].begin(), avai[j].end()));
                                avai.erase(avai.begin() + j);
                                break;
                            }
                        }
                        break;
                    case 1:
                        if (size_t > j + 1 && avai[j].size() == avai[j + 1].size() && avai[j][0] + 2 < avai[j + 1][0])
                        { // 考虑回手
                            for (int k = i + 1; k < 8; k++)
                            {
    ...
    { // 除大王外
        ...
        case 1:
        {
            mycounts[i] = 0;
            set<Card>::iterator it_card = myCards.lower_bound(i * 4);
            BestCombo.push_back(new CardCombo(*it_card));
            myCards.erase(*it_card);
            break;
        }
        case 2:
...
void GenerateBestCombo(CardCombo t)
{
    ...
    if (t.comboType == CardComboType::PASS)
    { // 农民要地主的牌，地主要所有人的牌
        if (MAX > MAX_CHOICE + 30 * (lastplayer == landlordPosition || myPosition == landlordPosition || cardRemaining[myPosition] < 4) + 50 * ((lastplayer == landlordPosition || myPosition == landlordPosition) && cardRemaining[lastplayer] <= 2))
        {
            MAX_CHOICE = MAX;
            BestSolve = t;
        }
...
CardCombo &Play_Strategy()
{
    ...
    else
    {
        ...
        for (; it_combo != BestCombo.end(); ++it_combo)
        {
            if ((*it_combo)->comboType == CardComboType::STRAIGHT)
            {
                if (myPosition == landlordPosition && ifIcanbeBeaten(**it_combo) || ((*it_combo)->cards.size() > 7 || (*it_combo)->comboLevel == MAX_STRAIGHT_LEVEL))
                { // 1处加要不起的tag
                    return *BestCombo[0];
                }
                else
                { // 若要的起考虑回手
                    ...
                    {
                        if ((*it)->cards.size() == (*it_combo)->cards.size() && ((*it)->comboLevel - (*it_combo)->comboLevel > min(4, MAX_STRAIGHT_LEVEL - (*it_combo)->comboLevel)))
                        { // 有回手
                            return *BestCombo[0];
                        }
            ...
            {
                if (myPosition == landlordPosition && ifIcanbeBeaten(**it_combo))
                { // 加要不起的tag
                    return *BestCombo[0];
                }
                else
...
// 判断是否合法
bool checkValid(CardCombo solve)
{
